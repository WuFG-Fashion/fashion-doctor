...
1. **frontmatter 契约字段**（新建 wiki/ 页，在原有 aliases/confidence/brand_specific 之外补齐）：
   ...
   - `volatility: evergreen | slow | fast | perishable`——方法论/框架 = evergreen；财报/季度经营 = slow；门店/渠道/联名/人事 = fast；价格/促销/库存状态 = perishable（技术实践类多为 evergreen，含版本号数据按 slow 处理）
   - `as_of: YYYY-MM-DD`——信息观察时点（**不是写入日期**；如"DuckDB 0.10 基准"观察时点=该版本发布/评测日）
