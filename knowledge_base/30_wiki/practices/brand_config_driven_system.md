...
1. 配置驱动是多品牌系统的**唯一可持续扩展方式**：新增品牌只改配置、不改代码，这与 [[brand_config]] 的"配置器"设计闭环，也是 [[system_architecture]]"配置驱动"原则的落地实现。
2. 关键风险在**字段映射层**：各品牌库字段命名不一致（`shop_name` vs `store_name`）时，映射若散落在查询里而非集中一处，就会出现同一指标多处计算、口径悄悄分叉——应集中到单一映射真源。
3. "配置驱动"与"数据隔离"必须同时设计：品牌配置决定加载范围，权限决定可见范围，两者混用会导致跨品牌越权（参见 [[cross_brand_integration]] 数据权限小节）。
4. 落地判据：**新增一个品牌所需的代码改动应为 0**——若仍需改代码，说明抽象层级没做够，而不是"这个品牌特殊"。

## 信息链

- 上游来源：[[brand_config]]（品牌配置管理概念页） · [[system_architecture]] → 本页（[[brand_config_driven_system]]）→ 下游应用：[[multi_brand_unified_analytics]] · [[cross_brand_integration]] · [[data_model]]

## 前沿

1. **待补**：本页缺少"新增品牌的操作清单"（配置字段 → 建库 → 导数据 → 前端出现 Tab 的逐步 checklist），目前只讲了原理。
2. **待验证**：配置热更新（`reload_configs`）在生产环境下的**原子性**——若重载过程中有查询在跑，是否存在读到半套配置的窗口。
