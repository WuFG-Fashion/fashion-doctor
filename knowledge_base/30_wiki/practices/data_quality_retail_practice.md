...
1. 数据质量的**唯一有效机制是前置红线 + 事后抽检**：红线（日期格式/金额非负/SKU 非空）在导入时拦截，抽检在导入后验证——两者缺一，错误数据都会流到看板层被当成事实。
2. 零售数据的特有陷阱是**口径而非脏值**：同一天两个销售额往往不是谁写错了，而是口径不同（含/不含退货、吊牌/实收）——因此质量检查必须同时校验"数值自洽"与"口径标注"。
3. 退换货是最容易污染质量线的场景：退货若直接记为负金额，会与"金额字段禁止负数"的红线冲突——本页给出的处置是**退货单独处理**，这一约定需要与 [[data_model]] 的字段约束保持一致。
4. 卡宾侧的落地锚点：可用 [[零售数据仓库SQL实践]] 的"数据质量监控 SQL"（空值率检查）作为每日自检的模板，把红线写成可执行的查询。

## 信息链

- 上游来源：[[data_model]]（字段命名规范与质量红线） · [[data_quality]] → 本页（[[data_quality_retail_practice]]）→ 下游应用：[[multi_brand_unified_analytics]] · [[bi_dashboard_retail_deployment]] · [[零售数据仓库SQL实践]]

## 前沿

1. **待补**：本页的检查项尚未脚本化——需要一份可直接运行的**每日数据质量自检脚本**（覆盖空值率/格式/重复/口径自洽四类），并接入现有更新流程。
2. **待验证**：多品牌场景下"同一指标跨品牌口径不一致"的**自动检测方法**尚未定义（当前依赖人工比对），是可沉淀为规则的部分。
