...
1. 本页的核心不是"SQL 写法技巧"，而是一条**性能基线的量化对照**：日销售汇总 0.5s→0.05s、RFM 计算 120s→3s——**优化收益的 90% 来自索引策略，而非改写技巧**，这是先建索引再谈改写的顺序依据。
2. 五个场景指向同一套动作：**复合索引按"过滤列优先、覆盖列在后"排列**（如 `idx_shop_date_amt` 把 amount/customer_id 也纳入以覆盖索引免回表），这是可以直接搬进卡宾库的硬规则。
3. RFM 一条最值得记：**窗口函数替代自连接**（`NTILE(5) OVER (...)`），把百万级会员从分钟级压到秒级——与 [[vip_tier]] 的 RFM 分层方法论在同一条链上。
4. SQLite 场景需注意适配：本页示例含 `DATEDIFF`/`NTILE` 等 PostgreSQL 语法，卡宾现行库为 SQLite，**移植时须替换为等价写法**（日期用字符串比较、分位用自连接或 Python 侧计算），否则会误判为"语句报错=数据问题"。

## 信息链

- 上游来源：[[2026-06-06_腾讯云社区_MySQL查询优化]] · [[2026-06-06_百度开发者_SQL优化实战]] · [[2026-06-30_Dupple_SQL查询优化2026_PostgreSQL18]] · [[2026-06-30_GeeksForGeeks_SQL查询优化十大实践2026]] → 本页（[[零售数据仓库SQL实践]]）→ 下游应用：[[SQL查询性能优化]] · [[multi_brand_unified_analytics]] · [[data_quality_retail_practice]] · [[data_model]]

## 前沿

1. **待补**：本页性能基线全部为 PostgreSQL/MySQL 语境的示例值，**缺少 SQLite 实测对照**——卡宾现行库的同类查询实际耗时需要一次实测回填，否则基线无法作为验收依据。
2. **待验证假设**：复合索引 `(shop_id, sale_date, amount, customer_id)` 在写入频繁的销售表的**写放大代价**是否可接受——需在卡宾实际数据量下对比优化前后的导入耗时。
