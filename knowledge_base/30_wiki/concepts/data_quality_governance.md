...
1. **四大质量指标在本系统的优先级必须重排**：页内建议电商零售优先「时效性+完整性」，但多品牌异构库的现实痛点是「一致性」（跨品牌字段口径）——FIELD_MAP 映射层的存在本身就是一致性问题已成现实的证据，本系统一致性权重应最高，这是对页内通用结论的场景化修正。
2. **多品牌场景的质量主战场是标准统一而非单点清洗**：超过 60% 多工厂协同障碍源于数据标准不统一（同一物料在不同系统编码不同、同一指标不同部门定义不同）——治理投入应先花在编码与口径统一上。
3. **「监控→预警→闭环→迭代」四步可直接落为日更校验规范**：卡宾/太平鸟库的日更校验按此四步脚本化（字段完整性扫描→异常告警→根因修复→规则迭代），是本页对本系统最高优先级的落地动作，与 [[etl_governance_convergence_2026]] 的治理前移互为表里。
...
**适用边界**：四大质量指标（完整性 / 准确性 / 一致性 / 时效性）中，本页建议电商零售优先「时效性 + 完整性」——但对本系统的**多品牌异构库**场景，「一致性」（跨品牌字段口径）权重应最高：FIELD_MAP 映射层的存在正说明一致性问题已是现实痛点。

**可迁移性**：「监控 → 预警 → 闭环 → 迭代」四步可直接落为本系统的数据校验脚本规范（如卡宾 / 太平鸟库的日更校验）。

**下轮核查**：与 `data_asset_management_2026` 的分工（本页管质量专项，后者管资产全流程）；四指标的量化定义是否需要按本系统字段（如 `v_sales` 视图）实例化成 practices 页。
