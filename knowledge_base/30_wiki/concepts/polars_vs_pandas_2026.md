...
1. **「三引擎协同」结论成立，但收益触发点在百万行级**：10 万行以下几乎无差别、性能差距从 100 万行开始显著（排序 11 倍/CSV 加载 5 倍且省 87% 内存）——本系统当前量级 Pandas 即够用，Polars/DuckDB 的收益在多品牌季度汇总（100-500 万行）才触发。
2. **Stars 数存在 32K 与 80K+ 两说，差 2 倍以上**：开源项目 star 数公开可查，引用时以 GitHub 官方仓库实时数为准并标注查询日期——已登记知识库矛盾项，两说均不得直接引用。
3. **选型决策变量是数据规模 + 生态需求，不是趋势**：ML 生态/Excel 支持选 Pandas、ETL 流水线/内存受限选 Polars、多表 JOIN/联邦查询选 DuckDB（见 [[duckdb_olap_engine_2026]]）——Modin/Vaex 两端不靠不推荐，新项目趋势向 Arrow 统一内存格式（见 [[arrow_zero_copy_interop_2026]]）。
