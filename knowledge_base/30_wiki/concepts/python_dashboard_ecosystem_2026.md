...
1. **看板选型第一问是「给谁用」**：内部原型选 Streamlit（1-3 天交付）、企业级多页面选 Dash（1-2 周）、对外多租户嵌入 Metabase 类平台（1-5 天 vs 自建 4-8 周）——交付周期差距一个数量级，场景错配是看板项目返工的第一根因。
2. **V3 的 FastAPI+静态 SPA 与 Streamlit 不是替代关系**：Streamlit 适合纯内部快速原型（但多用户状态管理与大表渲染是硬伤），V3 走前后端分离利于移动端与缓存版本控制——两路线并存分场景，引用本页时勿表述为互斥。
3. **Streamlit 生产三大失效模式要前置防**：模块级全局状态泄漏（跨会话数据污染）/大表超 5 万行渲染崩溃/玩具数据幻觉（3 行演示 5 万行卡死）——多用户生产场景应考虑 Dash（回调无状态）或维持静态 SPA 路线；Streamlit 1.55+ 每两周发版，版本引用注意时效。
...
**适用边界**：「内部看板首选 Streamlit（1-3 天交付）」与本系统 V3 现状（FastAPI + 静态 SPA）不同构——V3 走的是前后端分离路线（利于移动端与缓存版本控制），Streamlit 路线适合**纯内部快速原型**；两路线不是替代关系，引用时须分场景。

**下轮核查**：「多租户 SaaS 应嵌入平台（1-5 天）而非自建（4-8 周）」的工时口径来源（经验值 vs 实测）；与 `streamlit_dashboard_2026`（单工具深潜）、`retail_bi_visualization_2026`（BI 工具选型）的分层关系。
