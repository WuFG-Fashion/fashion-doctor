...
1. **CRISP-DM 七步与常态化仪表盘互补而非替代**：七步法适合项目制专项分析（如品类滞销归因、某波段复盘），仪表盘适合日常监控——本系统两种模式都需要，用一种覆盖另一种会导致专项没流程、日常没看板。
2. **服装五维指标体系可作分析项目的指标骨架**：商品（爆款识别 + 新品 30 天存活率）/销售（售罄率 30 天<30% 危险）/库存（库龄>90 天死库存、断码率）/渠道（ROI/退货率）/用户（复购率）——阈值与 kb_benchmarks 对接后成为可执行的分析起点，直接服务 CRISP-DM 第 1 步「定义问题」。
3. **进销存 SQL 六种写法覆盖约 85% 业务问题**：LEFT JOIN+COALESCE / 窗口函数时点库存 / CTE 分层 / 分组聚合 / 条件聚合 / 物化快照，综合 IO 降 68%、吞吐升 2.3 倍——把六种写法按本系统 v_sales 视图实例化成 practices 页，比引入新工具更划算。
4. **本页管流程不管引擎**：三引擎分工详见 [[python_data_stack_decision_2026]] 与 [[polars_vs_pandas_2026]]，避免四页互相复述——引用本页时按「流程骨架 + 指标体系」取用。
...
**适用边界**：CRISP-DM 七步是**项目制分析**流程（一次分析一个业务问题），与本系统「常态化仪表盘」模式不同——七步法适合专项（如某品类滞销归因），仪表盘适合日常监控，两者互补而非替代。

**可迁移性**：「按数据规模选引擎」与 `python_data_stack_decision_2026`（三重边界）、`polars_vs_pandas_2026`（三引擎）重复覆盖——本页应聚焦「流程」，引擎选型交给专门页，避免四页互相复述。

**下轮核查**：EDA 三类分析 + 可视化清单是否需要按本系统 `v_sales` 视图实例化为 practices 页；与 `retail_analytics_reporting_2026`（报表体系）的分工边界。
