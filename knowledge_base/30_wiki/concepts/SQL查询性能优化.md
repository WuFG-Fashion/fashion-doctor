...
1. **优化收益的 70% 来自索引与改写，参数调优是末位**：索引设计 + SQL 改写占优化效果约七成——先建复合索引（最左前缀原则，如销售表 shop_id+sale_date+category）、再改五大反模式（SELECT * / OR 跨列 / WHERE 函数 / N+1 / 隐式类型转换），最后才谈参数；顺序反了会白做。
2. **反直觉红线：索引越多越慢**：每个索引都拖慢写入并占用存储——须季度审计删除未使用索引，这与「加索引解决慢查询」的直觉相反，是本页最该固化的运维纪律。
3. **基准数字必须带量级引用**：40 倍提升（3.2 秒→0.08 秒）发生在百万行级，10 万行级只有 10 倍——本系统单品牌日销售远低于此量级，SQLite 上实际体感差距小；引用性能基准不带量级前提会误导选型。
4. **方言过滤：PostgreSQL 特性对本系统不适用**：本页大量 PG 18/19 内容（跳过扫描、异步 I/O、GROUP BY ALL、SIMD COPY）对现役引擎 SQLite 无效——本页对本系统的可用部分是「索引 + 改写」通用层，方言层仅作引擎升级时的选型储备。
...
**适用边界**：SQL 优化技巧分「索引 / 执行计划层」（通用）与「特定引擎方言层」（SQLite / PostgreSQL / MySQL 各异）——本页若未标注引擎，则通用技巧可直接用，方言技巧需按本系统实际引擎（SQLite）过滤。

**下轮核查**：① 与 `SQL查询性能优化` 相关的实操页面（本系统 `v_sales` 视图、多品牌异构库查询）是否已建实践页沉淀；② 本页示例是否覆盖 SQLite 的限制（如部分窗口函数版本差异）。
