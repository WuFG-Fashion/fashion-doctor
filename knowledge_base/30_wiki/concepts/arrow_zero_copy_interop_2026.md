...
1. **Arrow 是标准不是工具**：定义跨语言统一列式内存布局（「USB-C 接口」类比准确），让 Polars/DuckDB/Pandas 共享同一物理内存、消除序列化三重浪费——它不替代任何数据库，是工具间零拷贝的前提条件。
2. **性能收益随数据量与工具交换频率增长，本系统尚未触发**：序列化对比（IPC 比 Protobuf 反序列化快 48 倍）与零拷贝转换（90 倍）均基于千万行级——本系统 SQLite 单机 + Python 轻量栈量级下收益未触发，当下定位是架构储备知识，不为用而用。
3. **迁移触发点明确，届时自动升级为在用技术**：Pandas 3.0 已 Arrow-backed（字符串后端内存约降 85%、str 操作提速 30 倍）、Polar/DuckDB 互转默认走 Arrow——若未来切 Polars/DuckDB 栈（见 [[polars_vs_pandas_2026]] 与 [[duckdb_olap_engine_2026]]），Arrow 自动成为默认底座，无需单独决策。
