...
1. **DuckDB 的切换理由是省工而非性能**：窗口函数快 10 倍依赖百万行级，本系统量级下与 SQLite 体感差距小——「直接查询 CSV/Parquet 免导入」与「ATTACH 多库联邦查询」（跨品牌异构库聚合）才是对本系统的真实价值点。
2. **三引擎混合栈是 2026 最佳实践，但对本系统是渐进可选**：DuckDB（SQL 准备）→Polars（特征工程）→Pandas（ML/可视化）经 Arrow 零拷贝串联（见 [[arrow_zero_copy_interop_2026]]）——SQLite 现役栈不构成技术债，切换决策挂在数据量级触发点上，与 [[python_data_stack_decision_2026]] 的边界框架一致。
3. **边界清晰：分析侧专用，不能接管在线服务**：非 OLTP（不适合高并发写入）、单机为主（无原生分布式）——V3 的在线服务写入场景不能交给 DuckDB，它只适合替换分析与报表侧的查询层。
...
## DuckDB Python 嵌入式分析范式（2026-08新增）⭐

> 来源：[[2026-08-09_DuckDB官方_v1.5系列与Python嵌入式分析范式]]

### 双轨维护 + v1.5.4 生态对接
- v1.5 起双轨：v1.4 LTS（Andium）+ v1.5 current；**DuckDB 2.0 计划 2026 年 9 月**，v1.4.5 向后兼容、升级无需手动迁移数据文件。
- v1.5.4 纳入 **ADBC 1.1.0 + Rich Error API**、Avro 扩展、Unity Catalog；MERGE INTO 语义修正；30+ Bug 修复。

### Python 三套 API
| API | 风格 | 适合 |
|-----|------|------|
| DB-API | 标准 execute/fetch | 通用脚本 |
| Relational API | 链式调用，近 Pandas | 探索式转换 |
| Spark API | PySpark 风格 | Spark 迁移用户 |

与 Pandas/Arrow **近乎零序列化换手**：`duckdb.sql("... FROM my_df ...").df()` / `.arrow()`；内存模式（进程级临时）与持久化模式（落盘反复查询）并存；核外可直查数十 GB Parquet。

### 对多品牌分析架构的衔接
- Relational API 与 [[python_sql_integration_patterns_2026]] 的 SQLAlchemy 生产模式互补：探索用 DuckDB SQL、生产用 SQLAlchemy 原生。
- ADBC 1.1.0 + Unity Catalog 让 DuckDB 可作多品牌统一数据层本地查询引擎，与 [[multi_brand_unified_analytics]] 的 Iceberg/湖仓路线协同。
...
**适用边界**：「窗口函数比 Pandas 快 10x」依赖数据量级（百万行级才显著）——本系统当前量级下 DuckDB 与 SQLite 的体感差距小；切换决策应基于「直接查询 CSV / Parquet」的省工价值，而非纯性能。

**可迁移性**：「DuckDB → Polars → Pandas 三引擎混合栈」与 `polars_vs_pandas_2026`、`python_data_stack_decision_2026`、`arrow_zero_copy_interop_2026` 构成选型四件套，四页互引关系是 RAG 检索「技术选型」类问题的主干链路。

**下轮核查**：版本时效（DuckDB 迭代以月计，性能数字保鲜期短）；与 V3 现有 SQLite 栈的共存方案（渐进替换 vs 双栈并行）是否需要单独立项页。
