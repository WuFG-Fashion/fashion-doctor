...
本页判断 2026 年 ETL/ELT 之争已收敛为「90% 中大型企业单平台双模式」：主数据与敏感报表走 ETL 前置清洗、行为日志与实时分析走 ELT 入仓后转换，全场景增量统一靠 CDC。服装零售直接相关的三条「必须 ETL」判据是：日数据百万条以内的中小零售、主数据 MDM 统一分发（保证唯一主键与统一编码）、源系统算力薄弱需限流分批抽数。架构上电商侧标准解法是「源 → ELT 摄取 → 数仓 → dbt 三层建模」四层栈（staging / intermediate / mart），其价值在于把「这个收入数字从哪来」从口头传承变为 dbt 血缘图。最贵的错误是两套管道写同一报表层的混合陷阱，另有两条隐形雷：供应商主数据最常被忽略、连接器可能静默失败三周无人察觉。

## 信息链

上游来源 [[2026-08-06_ETL_ELT_ETLT混合架构与电商数据工程四层]] → 本页（ETL/ELT 选择判据与 dbt 三层建模） → 下游应用 [[python_data_stack_decision_2026]]、[[multi_brand_unified_analytics]]
...
待验证：dbt 三层结构（staging/intermediate/mart）与本项目分析层的现有分层映射关系尚未梳理，可考虑在数据基建侧做一次分层对齐。
