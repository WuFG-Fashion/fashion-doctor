...
跨页提示：本页已自标数据矛盾——服装 specialty 季末售罄 60-80%（行业基准区间，上沿 80%）vs kb_benchmarks 的 end_season_target=0.8 与 season_end_sell_through.excellent=0.7。本页判为「区间 vs 目标」的口径差异；但 0.8 同时出现在基准上沿与库内进取目标两处，若后续按 kb 0.8 考核而基准认为 0.8 只是区间极限，会系统性抬高目标，此项待裁定。

待核实：本页基准（服装 specialty 周度 8-12%/季末 60-80%、快时尚 15-25%/85-95%、鞋类 6-10%/55-75%、美妆 10-15%/70-90%）来自单一来源 aislestock，其自称「行业参考：NRF research library + 运营商演示」，但未逐项给出对应关系；属单一信源，未与库内 retailnorthstar 页的分品类正价售罄（核心 80-90%/延续 65-78%/新款 60-75%）交叉验证——两套分品类基准若能对齐，可信度才可升级。

待核实：本页「<8% 第 4 周触发降价复核」是阈值型规则，但第 4 周的选取依据未给出；案例中 1200 件春装售 192（16%）为演示数，非实测样本。

待观察：本页属方法论页，适用边界为 specialty 服装，快时尚指标差异大（SKU 寿命 4-6 周），跨品类套用需重设阈值。
