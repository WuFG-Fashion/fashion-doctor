...
PostgreSQL 19 Beta 1 的 60+ 项新特性中，对零售数据分析直接有价值的集中在四处：GROUP BY ALL 简化多维分析 SQL、Anti-Join 自动优化提振「未购某品类会员」类查询、IGNORE NULLS 解决销售日期缺失补值、SIMD COPY 加速百万级流水导入。这些特性的共同指向是——把过去需要应用层处理的逻辑下沉到数据库，减少数据搬运。

## 信息链

[[SQL查询性能优化]]（SQL 三维优化法）→ 本页（PG19 新特性）→ [[零售数据仓库SQL实践]]（零售 SQL 模板）与 [[ETL架构选型]]（数据导入性能）
...
跟踪 PG19 正式发布（预计 2026 年 9-10 月）后各特性在托管服务（腾讯云/阿里云）的可用时间；评估 IGNORE NULLS 在补值场景的实际收益。
