...
DuckDB 官方给出的查询优化栈分三层且加速量级差异巨大：文件级 Hive 分区 + Glob（10-365x）、行组级谓词下推 + 行组调优（2-15x）、库级 Filter Index + 物化表（5-100x）。最具实操价值的规则是——物化预聚合可让 1B 行聚合查询只扫 168 行、毫秒返回，而一旦内存溢出落盘则会慢 10-100x，因此避免落盘比任何索引技巧都重要。同时须警惕阻断谓词下推的反模式（对日期字段 CAST 后 LIKE、日期加 INTERVAL 比较、超大 IN 列表）。

## 信息链

上游来源 2026-08-12_DuckDB官方_查询性能调优三层级实战 → 本页（DuckDB 三层优化栈与反模式） → 下游应用 [[duckdb_olap_engine_2026]] 引擎实践 / [[SQL查询性能优化]] 索引与改写 / [[2026-08-15_SQL优化2026向量化执行与PG18_DuckDB基准]] 基准对照
...
待验证：物化表模板（按小时预聚合）适用于本项目"每日指标预计算"，但需评估预计算窗口的选择——服装销售以日/周为粒度，按小时预聚合可能过度细分导致无效存储。
