...
50GB 实测显示引擎选型已从"二选一"转为"混合用"：Polars 2.1.0 在 CSV Join 上比 Pandas 3.0.1 快约 12.4x、内存低 60%（GroupBy 峰值节省 73%），DuckDB 1.2.3 比 Spark 4.0 延迟低 89%；自建 Polars+DuckDB 栈比托管方案（Fivetran+Snowflake）成本低至 $0.03/GB vs $0.18/GB 且快 12x。选型规则清晰——数据 >100MB/ETL/新项目/多核/流式走 Polars，遗留 pandas/重 sklearn·matplotlib/<50 万行走 Pandas，三者经 Arrow 零拷贝串联。

## 信息链

上游来源 2026-08-12_Polars2.1_Pandas3.0_生产级性能对比 → 本页（Polars/DuckDB/Pandas 生产级基准与选型） → 下游应用 [[python_data_stack_decision_2026]] 技术栈决策 / [[arrow_zero_copy_interop_2026]] Arrow 互操作 / [[polars_vs_pandas_2026]] 引擎选型 / [[duckdb_olap_engine_2026]] DuckDB 应用
...
待验证：基准为 johal.in 单方实测且硬件（32GB RAM）接近内存上限，结论可能受换页影响——建议用自家真实数据规模（多品牌周报量级远小于 50GB）验证是否仍存在同量级差异。
