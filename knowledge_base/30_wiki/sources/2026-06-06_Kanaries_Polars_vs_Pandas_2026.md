...
> **原始文件**：`10_web/articles/2026-06-06_Kanaries_Polars_vs_Pandas_2026.md`
...
本页的核心结论是 Polars 的优势随数据量放大而扩大（排序场景 11 倍、CSV 加载内存省 87%），劣势则是生态成熟度与 Excel 支持不足。本库处于单机环境、数据量未达百万行，因此当下应以生态与可维护性优先，把 Polars 列为数据量增长后的升级选项，而非立即迁移。

## 信息链

上游来源 [[2026-06-06_Kanaries_Polars_vs_Pandas_2026]] → 本页 → 下游应用 [[polars_vs_pandas_2026]]、[[SQL查询性能优化]]、[[零售数据仓库SQL实践]]

## 前沿

跟踪本库数据行数增长曲线，接近百万行量级时再评估迁移 Polars。
