...
来源存疑：本页「入闸捕获问题的修复成本仅是生产暴露后的 1/10」引自 IBM《Poor Data Quality 成本》研究，属经典引用转述；数据可观测性四支柱与工具链清单来自行业综述（PADISO / Branch8 / SysGenPro 等），均为方法论汇编而非单点实证。

待核实：本页给出的 dbt 入闸示例（assert_positive_order_totals、schema.yml 测试）为通用零售骨架，本项目实际字段（shop_code / sku_code / 品牌隔离）需按 [[brand_config_driven_system]] 的字段映射改造；示例中的 refund 例外分支说明「一刀切校验」在退货场景会误报，落地前须逐品牌确认业务例外。

待观察（适用边界）：本页建议每数据集回答「谁拥有 / 何时校验 / 是否批准用于 AI 训练」三问，本项目现有品类映射表与品牌注册表仅是目录雏形，尚无 owner 与校验日期字段；治理左移的动作项在完成字段补齐前不具备可执行性。
