...
服装行业指标体系应围绕商品/销售/库存/渠道/用户五维构建，配"定目标→盘数据→定指标→分场景"四步法落地；工程侧则用 raw→stg→mart 三层数仓 + 星型模式 + SCD Type 2 把口径固化为约束，进销存 SQL 六种写法可覆盖约 85% 业务问题。其女装 200 店/年销 8 亿案例给出可验收的目标集：售罄率（90天）>70%、库存周转 <90 天、库龄>90天占比 <15%、复购率（180天）>35%、会员贡献 >60%，上线一年实际达成售罄率 58%→73%、周转 112→85 天、库龄 22%→12%、复购 28%→38%。

## 信息链

上游来源 2026-08-09_CSDN_服装行业指标体系五维框架与电商数仓分层建设 → 本页（服装五维指标体系与数仓分层） → 下游应用 [[retail_analytics_reporting_2026]] 报表体系 / [[sku_fine_management]] 款色码下钻 / [[data_asset_management_2026]] 标准库 / [[kb_benchmarks.json]] 指标阈值
...
待验证：案例中的目标值与实绩（如售罄率 58%→73%）为单一女装品牌口径，不同品类差异大——建议作为方法模板引用，但阈值须按自家品类重设后再写入知识库基准。
