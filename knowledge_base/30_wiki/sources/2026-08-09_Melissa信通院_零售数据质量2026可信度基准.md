...
aliases: ["零售数据质量", "2026：从\"批后清洗\"到\"情境可信度", "零售数据质量 2026：从\"批后清洗\"到\"情境可信度"]
confidence: 媒体估算
...
2026 年零售数据质量的定义已升级为"情境可信度"（contextual trustworthiness）——即可即时、自信、合规地采取行动的数据，考核重心从"技术建设完成率"转向"业务价值转化率"。信通院白皮书给出 DQS 四项硬基准：完整性 ≥99.5%、准确性 ≥99.9%、跨系统一致性差异 ≤0.1%、及时性 T+0 占比 ≥80%；零售场景另有库存预测偏差 ≤10%、数据时效分钟级（促销秒级）等要求。

## 信息链

上游来源 2026-08-09_Melissa信通院_零售数据质量2026可信度基准 → 本页（零售数据质量 DQS 基准与情境可信度） → 下游应用 [[data_quality_governance]] 四维框架 / [[data_quality_retail_practice]] 实践基准 / [[retail_data_workflow_2026]] 采集点校验 / [[kb_benchmarks.json]] 质量阈值
...
待验证：四项 DQS 基准为行业通用要求，本项目多品牌异构库（各品牌字段口径不同）当前实际达标率未知——建议先做一次数据质量盘点，量化差距后再定改进优先级。
