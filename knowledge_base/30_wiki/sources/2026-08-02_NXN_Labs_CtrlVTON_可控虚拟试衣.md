...
本页记录虚拟试衣从被动换衣到主动穿衣的范式推进：现有 VTO 本质是图像修补（inpainting）换装，能力局限于穿上走秀/转身，且存在擦除残留、身份丢失、形状误导等失败模式。NXN Labs 与 KAIST 提出的 CtrlVTON 可控换衣框架 + VIP-SAM 服装分割，让用户可精确控制扎进/拉链/内外层等穿衣语义，把 VTO 从自动生成一个穿新衣的人推进到可控制具体穿法。其技术判断是：VTO 的下一阶段竞争点不在生成质量而在可控性——用户能否表达并实现具体的穿衣意图。

## 信息链

上游来源 [[2026-08-02_NXN_Labs_CtrlVTON_可控虚拟试衣]] → 本页（可控虚拟试衣技术路线） → 下游应用 [[ai_fashion_consumer_2026]]、[[apparel_ai_agents_2026]]
...
待观察：CtrlVTON 属研究阶段框架，商业化产品形态与推理成本尚未披露。
