# -*- coding: utf-8 -*-
"""Round B (L2_03/04/05): v3 frontmatter normalizer.
Preserves original [[...]] wrapper of cross_refs tokens; appends new [[name]] links.
Preserves sources single-line/block-list format."""
import re, os
...
def extract_crossref_full(line):
    """Return list of full [[...]] token strings (preserve wrapper & |alias)."""
    start = line.index('['); end = line.rindex(']')
    body = line[start+1:end]
    toks = re.findall(r'\[\[[^\[\]]+\]\]', body)     # well-formed
    toks += re.findall(r'\[\[[^\[\]]+\]', body)       # malformed single-close
    seen = set(); out = []
    for t in toks:
        key = t.strip('[]')
        if key not in seen:
...
def update_file(relpath, new_srcs):
    ...
    while i <= end:
        ...
        if re.match(r'^(\s*)sources:\s*\[.*\]\s*$', line):
            ...
            toks = extract_crossref_full(line)
            for s in new_srcs:
                t = f"[[{s}]]"
                if s not in [x.strip('[]') for x in toks]:
