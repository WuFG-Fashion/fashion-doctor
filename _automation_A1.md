...
1. 读取 `kb_benchmarks.json` 的 `focus_brands`，取出本组 12 个 key，解析到 `30_wiki/entities/` 页（别名映射：uniqlo_fast_retailing→fast_retailing、gxg_muson→muson_gxg、zara_inditex→inditex_zara；其余同名）。检查每个是否有实体页 + 至少 3 篇 `30_wiki/sources/`；标注「缺实体 / 少源 / OK」——**此标注仅用于排序优先级，不影响覆盖**，OK 品牌仍须全维度检索核验。⚠️ 缺口清单**仅围绕本组 12 品牌**，不得扩散到 A2/A3 品牌或非焦点竞品。
...
git add knowledge_base/10_web knowledge_base/30_wiki knowledge_base/90_meta/_health knowledge_base/90_meta/__index__ && git commit -m "[auto] Round A1 — L2_00/01/02 (分组A1·品牌全维度)" && git push
```

## 第八步：写日志 + 每日健康快照
1. 在 `knowledge_base/30_90_meta/log.md` 追加：| YYYY-MM-DD HH:MM | ingestA1 | L2_00/01/02 — 采集X篇/织网X条/矛盾X处 |
...
### 9.3 分段提交（替换原第七步单次提交）
- 前半程（品牌 1-6 写完）：`git pull --ff-only || true && git add knowledge_base/10_web knowledge_base/30_wiki knowledge_base/90_meta/_health knowledge_base/90_meta/__index__ && git commit -m "[auto] Round A1 — 前半程(品牌1-6)"`
- 后半程（品牌 7-12 写完）：`git pull --ff-only || true && git add knowledge_base/10_web knowledge_base/30_wiki knowledge_base/90_meta/_health knowledge_base/90_meta/__index__ && git commit -m "[auto] Round A1 — 后半程(品牌7-12)" && git push`
