...
3. `python knowledge_base/tools/kb_updater.py` 重建 `__index__/master_index.json`（校验 kb_version 3.1 + 1400+ L3）。
...
- **矛盾基线（2026-09-06 对账，09-12 复核未变）**：全库（含 raw 副本）`⚠️ **数据矛盾**` = **57 页 / 66 处**（语义层 54 页 + wiki/raw 副本 3 页），`ℹ️ **基准核对**` = 129 处（09-12 已升至 156 处，+27 全部来自新页自带的「与 kb_benchmarks 一致 ✓」自洽标注，属健康增长，**应定性为「自洽标注数」而非疑似矛盾数**）。
  - ⚠️ **统计陷阱**：全库 grep 会命中 `_health/*` 快照与 `CLAUDE.md` 规则文本里的自引用 → **必须排除 `_health/` 与 `CLAUDE.md`**，否则得 71 页/82 处（假数）。
